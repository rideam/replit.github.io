...
First, we need to create a new Node.js Repl for our code. Navigate to [repl.it](https://repl.it/) and create a new Repl, selecting "Node.js" as the language. You may name the Repl however you wish.

We then want to add our only dependency, [timrjs](https://www.npmjs.com/package/timrjs). TimrJS is a library for creating timers in JavaScript. It saves us from implementing our own timer and comes with some cool features that we can use.

In the default `index.js` file that is included in the new Repl, add the following code:

`const Timr = require('timrjs');`

Press the "Run" button at the top and you should see Repl.it install the TimrJS library and adding it to the dependencies in the `package.json` file. Otherwise, you can also click the cube icon on the left panel- it allows you to search for packages by name - and search for "timrjs" then click the `+` icon that appears infront of the package name to install the package. 
...
The pomodoro timer program will be made up of a few separate functions. To begin with we have a function called `startPomodoro`, which when called starts a countdown of 25 minutes. 
...
When we click the "Run" button we want to output a message to show that the program is running. To do that place a `console.log` statement in the index.js file, below the require statement :
...
So we need to create the `startPomodoro` function. We can do this using the function declaration method. Declaring functions this way means that they are added to the top of the program. It therefore does not matter if the function is defined *after* it is called, because all function declarations are moved to the top of their scope before the code is executed. This is called [*hoisting*](https://www.w3schools.com/js/js_hoisting.asp).
...
In the `startPomodoro` function we start by creating an empty array called `totalPomodoros`. This array will hold the total number of completed pomodoro sessions.
...
When the countdown reaches zero the function must record a successful Pomodoro to the `totalPomodoros` array we have already created.
...
TimrJS returns an object which has several properties including `events`, `timer`, `running` and `options`, which is itself an object.

There are two key events in a TimrJS object, `ticker` and `finish`. Ticker is called every second, and emits an object that contains several properties. We are interested in:
- `formattedTime` and
- `percentDone`

`finish` is called once, when the countdown reaches zero and takes in a callback that is called when the timer is finished. 

TimrJS also has more customization options which you can check out on the library's [documentation](https://www.npmjs.com/package/timrjs) on npm. We will use it twice, in the `pomodoro` function and in counting down the break sessions.

Add the following code to the `pomodoro` function:
...
- Adding more control, for pausing, resetting etc.
